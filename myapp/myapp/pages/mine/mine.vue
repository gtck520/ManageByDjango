<template>
	<view class="mine animation-fade">
		...
		<view class="logo flexlogo" @click="goLogin" :hover-class="!login ? 'logo-hover' : ''">
			<!-- <image class="logo-img" :src="urls[0]" @click="clk(0)"></image>		 -->
			<avatar	
				selWidth="200px" selHeight="400upx" @upload="myUpload" :avatarSrc="url"
				class="logo-img">
			</avatar>
			<view class="logo-title flexlogo">
				<text class="uer-name ">Hi，{{login ? userinfo.mobile : '您未登录'}}</text>
		...
		<view class="cu-list menu sm-border card-menu margin-top bar-shadown">
			<view class="cu-item arrow" >
				<navigator :url="'children/share?m='+JSON.stringify(userinfo)">
				<view class="content">
...
<script>	
	import {pathToBase64,base64ToPath,} from '@/common/imagetool.js'
	...
	export default {
		...
		methods: {
			isLogin(){
				...
				this.loading = true
				this.url="";
				this.$api.getGlobalUser({noncestr: Date.now()}).then((res)=>{
					this.loading = false;
			...
			loginOut() {
				...
				if(_this.loginOutShow){
					try {
						uni.removeStorageSync("token"); //清空登录缓存
            ...
            myUpload(rsp) {
				...
				pathToBase64(rsp.path)
							.then(base64 => {
								...
								this.loading = true;
								var imgpath = base64;
								this.$api.setUserinfo(this.userinfo.id,{image:imgpath}).then((res)=>{
									this.loading = false;									
								}).catch((err)=>{
									this.loading = false;
									console.log('request fail', err);
